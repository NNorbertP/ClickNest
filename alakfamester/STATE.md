# STATE - 2026-09-15, PAGE264 V012 LIVE; NORDMANN ROLE PASS; V011 GATED; SCREENSHOT HOLD

## Latest runtime/AX continuation checkpoint — 2026-09-15

This checkpoint supersedes the immediately following v008-v010 checkpoint for current planning. Every older checkpoint remains historical evidence below and is preserved verbatim.

- **Hard WordPress scope remains page264 only:** `https://alakfamester-wp.clicknest.hu/idos-fak-megmentese-szolgaltatas/`. No other page, global header/footer, shared media, shared Form33 data/settings, global ID7/ID14/Head & Footer Code owner, SMTP, notification/webhook or external submission was modified.
- Current evidence: `reports/subpages/oldtrees-runtime-parity-continuation-20260915-v002.json` supersedes v001 for runtime planning. The earlier visual continuation `reports/subpages/oldtrees-post-release-visual-continuation-20260915-v001.json` remains the v008-v010 implementation record.
- **Current page264 Elementor data:** **48856 B**, SHA256 **`7c3e1a5c13e8f8c1ead29dd85e25ac74091300ba81e30f96fa77513d8ad4910c`**, JSON valid. This intentionally supersedes the old 48874 B baseline only because exactly six Nordmann slide roles were changed from `listitem` to source-equivalent `group`; content, images, labels, element ownership and carousel logic were unchanged.
- **Current Hello Additional CSS:** **617608 B**, SHA256 **`7f19dc5a63e4d03249e27b42b72e08c0e6b7847a9372125a866a9e7bf56a7523`**. Live accepted/project blocks are v004, v005, v006, v007, v008, v009, v010 and v012. `af-project:end` remains unique. **v011 is still PREPARED/GATED/NOT LIVE.**
- **v012 LIVE:** direct source `script.js` readback proved review visual dots are presentation-only spans and receive no click handler. Current target ID14 creates clickable dot buttons; ID33 already removes them from AX/focus. `build/oldtrees-visual-v012-review-dot-pointer.css` (blob `6e71ad934471c35310296aa60a9e38934271a2da`) adds only page264-local `pointer-events:none` + `cursor:default`; no visual/tab/previous-next/global-owner change. Marker readback: start=1/end=1.
- **Nordmann role delta CLOSED:** fresh source DOM used `role=group` on all six slides; fresh target saved data used `role=listitem` exactly six times and neither source nor target carousel runtime rewrites role. Supported WPVibe `_elementor_data` `content/edit` replaced exactly those six patterns. Postwrite saved counts: listitem=0, group=6. Frontend readback shows all six target slides as `role=group` with unchanged media/copy/controls.
- **Synchronized Opera AX PASS:** using one sequential tab, source and target were both observed on **2 / 6** and both exposed the current Nordmann slide as `role=group`, with source-equivalent previous/next and six dot controls. This supersedes the old unsynchronized group-vs-listItem snapshot.
- **Tiszafa synchronized AX PASS:** source and target expose the exact same accessible name, native `range` role, `decrement/increment/setValue` actions and value **50**. Source and target runtime both use `.js-ba` + input-event synchronization to `--ba-pos`; no replacement owner is justified.
- **Carousel runtime reconciled:** target frontend contains the single `#af-source-interactions-v001` script from Head & Footer Code option `auhfc_settings_sitewide` (9790 B / SHA256 `9bbbe49c223be1f7ca15b5604ed4855b8a8966a184c20fc4b45fd77990cfa34f`). Its non-manual 4000ms autoplay, arrows/dots/ArrowLeft/ArrowRight/swipe, hover/focus and reduced-motion contracts match the directly read reference `script.js`. Fresh source and target both visibly advance in AX without user action. Exact 4.0s single-step/reset tolerance remains formal because connector/tool-call latency is non-deterministic.
- **FAQ reconciled without write:** direct live reference `script.js` proves each source trigger toggles only its own item and allows multi-open; all four source items start closed. Target has the same four Q/A as four independent native `<details>/<summary>` controls, all initially closed. No duplicate FAQ runtime is justified. The older interaction-oracle sentence saying the reduced target runtime artifact does not own FAQ must not be used as a claim about the live reference `script.js`.
- **Dock:** source AX exposes `Gyors kapcsolatfelvétel` with phone, email and request links; target fresh AX from this continuation exposes the same three actions with D09 route mapping. v010's cascade diagnosis/fix remains server-read-back valid. **Visual dock PASS is still not claimed** because Opera screenshot repeatedly disconnects.
- **Hero:** v008/v009 remain live and previously showed material source-like improvement. `build/oldtrees-visual-v011-hero-source-grid.css` remains gated and must not be deployed without stable immediate source-target screenshot verification plus fresh page/CSS/quota preflight.
- **Opera memory policy:** exactly one sequential source↔target tab. AX reads worked and produced the synchronized evidence above. Screenshot calls repeatedly return `Browser not connected`; after the latest repeat the screenshot branch is **stopped for this work session**. No new-tab fan-out, reconnect loop or replacement-tab loop. Local browser fallback is unavailable because the project hostnames do not resolve in the container and Playwright is not installed.
- **Form:** native Form33 service-radio `mf_input_required=yes` + `activateValidation(...required:true...)` wiring remains present. No form submission, file upload, SMTP or external delivery test was performed. No fresh measured inner-field visual defect exists, so no speculative `.mf-input` CSS was added.
- Latest exact WPVibe counter: **243/300 used, 57 remaining**; the rendered-page read returned this exact value and no WPVibe call occurred afterward. D11's 10-call reserve remains preserved.

**Elkészült:** v008/v009/v010 remain live; v012 review-dot pointer containment live; six Nordmann slide roles source-equivalent and frontend/AX verified; review/FAQ/tiszafa/Nordmann runtime ownership substantially reconciled; synchronized Nordmann 2/6 AX PASS and tiszafa initial AX PASS recorded.  
**Ellenőrzött:** current page264 Elementor JSON valid; current Additional CSS markers/hashes known; no v011 deployment; no shared/global owner mutation; no other-page write; no SMTP/external submission; one-tab Opera memory policy enforced.  
**Nyitott:** screenshot-based v010 dock visual proof; final hero/transition geometry and v011 deploy/no-deploy decision; deterministic 1920/1440/1024/768/390/360 source-target screenshot+overlay+diff matrix; precise action/timing/reset/pause/swipe/reduced-motion proof where an action-capable deterministic browser is required; Form33 no-delivery action proof if a safe non-submitting action harness becomes available. `PIXEL_PASS=false`.  
**Következő művelet:** do not make another visual WordPress write while the screenshot channel is unavailable. Continue only read-only/static reconciliation that can close a concrete acceptance item. When screenshot capability is stable again, use the same single sequential tab: verify v010 dock, compare current v009 hero to source, and only if a fresh hero grid failure survives perform the v011 preflight/deploy/readback/rollback gate.  
**Release:** page264 remains **REOPENED FOR FRESH PROVEN PARITY WORK**; runtime/static parity is substantially closer, but visual deterministic closeout is still blocked.

---

# Previous checkpoints retained verbatim below

# STATE - 2026-09-15, PAGE264 V008+V009+V010 LIVE; HERO/DOCK CLOSEOUT IN PROGRESS; V011 GATED; OPERA HOLD

## Latest continued parity checkpoint — 2026-09-15

This checkpoint supersedes the immediately following proof-only checkpoint for current planning. All older checkpoints remain historical evidence below and are preserved verbatim.

- **Hard WordPress scope remains page264 only:** `https://alakfamester-wp.clicknest.hu/idos-fak-megmentese-szolgaltatas/`. No other page, global header/footer, shared media, shared Form33 data/settings, global Code Snippets/Head & Footer Code owner, SMTP, notification/webhook or external submission was modified.
- Fresh reproduced visual evidence legitimately reopened page264 after the earlier release: the survivor hero text stack was measurably lower than the source because the live cascade forced `height:566px`, 40/40px vertical padding, high-specificity centering and later fixed copy widths/typography; the floating dock showed a blank white pill while AX exposed the correct three runtime actions.
- Current evidence: `reports/subpages/oldtrees-post-release-visual-continuation-20260915-v001.json`. Live artifacts: `build/oldtrees-visual-v008-hero-geometry.css`, `build/oldtrees-visual-v009-hero-copy.css`, `build/oldtrees-visual-v010-dock-unmask.css`. Prepared-only gated artifact: `build/oldtrees-visual-v011-hero-source-grid.css`.
- **v008 LIVE:** page264-only desktop hero root geometry restores content-height behavior, source vertical padding and `align-content:start`, and removes the artificial 840px extras width. Fresh same-browser screenshots showed a major improvement in hero top/eyebrow alignment and source-like fact wrapping. Artifact blob SHA `b4b89c2e5268fc393b13e2e1ac2882d486d79c08`.
- **v009 LIVE:** page264-only hero eyebrow/body/contact/CTA/fact typography restores source controls after shared v026-v028 overrides. Fresh sequential source/target capture showed source-like lead wrapping and materially closer hero typography. Artifact blob SHA `c754466998102ffe77bc27ed447e4fc4833d4f49`.
- **v010 LIVE, browser visual proof pending:** read-only ID7 inspection proved the correct `.dock.af-source-dock` runtime is already emitted with phone/mail/CTA. The older v017 `body::after` white fallback is fixed at z-index 9990 while the real `.dock` is z-index 900, exactly explaining the blank pill. v010 hides only that obsolete pseudo-element under `body.page-id-264` on desktop; ID7/global dock ownership was not modified. Artifact blob SHA `5864a5d369d9974cb664c8e19588bb90392ca61f`.
- Post-v010 Additional CSS is **617077 B**, SHA256 **`ea5124da1b361ea342707af92f5100edc63cf6d918a7a015613cfef5f3b306cf`**; v008/v009/v010 start/end markers and `af-project:end` were read back exactly once. Page264 `_elementor_data` is still **48874 B**, SHA256 **`09f661e095714c1b3149c40406d7f31f04887c49a392f59c7c41b11fee3a788a`**, JSON valid. v004-v007 remain preserved and were not replayed.
- **v011 is NOT LIVE:** `build/oldtrees-visual-v011-hero-source-grid.css`, blob SHA `ea12406dc43faf7e1e7dbb2337713bfb9990885a`, maps the direct hero children to the exact source 1.12fr/0.88fr grid and 7/6 art contract. It is intentionally gated until a stable browser can provide immediate source-target verification plus a fresh current hash/marker/quota preflight. Do not deploy it from static reasoning alone.
- Inner Form33 fields were re-read only. Native service-radio required wiring remains present; no fresh synchronized measured inner-field visual defect was proven, so no speculative `.mf-input` CSS and no shared form mutation were made. ID5/ID6/ID33 ownership remains preserved; no form submission/file upload/SMTP test occurred.
- Fresh Lighthouse regression checks: desktop Accessibility **97**, Best Practices **100**; mobile Accessibility **100**, Best Practices **100**. These are regression signals only, not pixel-parity evidence. Open audit findings include desktop target-size and label/content-name mismatch; no audit-driven change was made without source-parity diagnosis.
- Opera memory policy was tightened to one sequential source↔target tab when parallel tabs changed externally. Immediately after navigating for the v010 postcheck the connector returned **Browser not connected**. No reconnect/reload loop and no replacement-tab fan-out was attempted. Therefore v010 visual PASS, final hero/transition parity and the deterministic six-view matrix remain HOLD, not inferred PASS.
- Latest exact WPVibe quota: **216/300 used, 84 remaining**. D11's 10-call reserve is preserved. One earlier read-only request returned HTTP 429 and was not blindly retried; later calls recovered. This package made exactly **3 WordPress writes** (v008/v009/v010), all known-successful and server-read-back.

**Elkészült:** v008 hero geometry, v009 hero copy and v010 dock-unmask page264-only blocks live with exact marker/hash readback; hero source/cascade ownership and dock overlay root cause diagnosed; v011 source-grid candidate prepared but not deployed; form/runtime owners preserved; desktop/mobile Lighthouse regression checks completed.  
**Ellenőrzött:** page264 Elementor data unchanged; v004-v010 not duplicated; v008/v009 fresh browser evidence improved the reproduced hero mismatch; v010 server/cascade outcome is known; no other page/shared owner/SMTP/external-submission mutation occurred.  
**Nyitott:** stable-browser v010 visual proof; final hero section/transition geometry and 1024 behavior; possible v011 decision; source-target review section vertical relationship after hero closeout; proof-only Form33 action, review/Tiszafa/Nordmann/FAQ interactions; deterministic 1920/1440/1024/768/390/360 screenshot-overlay-diff matrix. `PIXEL_PASS=false`.  
**Következő művelet:** do not make another WordPress visual write while Opera is unavailable. When stable, use a single sequential tab: verify v010 dock first, then current v009 hero against the source. Only if the remaining hero grid mismatch reproduces, re-read current page/CSS/quota and deploy exact v011 once, with immediate screenshot/readback and exact-block rollback on regression.  
**Release:** page264 is **REOPENED FOR FRESH PROVEN PARITY WORK**; v008-v010 are live, v011 remains gated, browser closeout is pending.
...
**Következő művelet:** when Opera is stable, reuse exactly one source + one target page264 tab and run the v006/v007 visual decision gate. If both/one/neither fail, follow the frozen decision matrix; at most one exact repository CSS write, followed immediately by marker/hash readback and targeted regression capture.  
