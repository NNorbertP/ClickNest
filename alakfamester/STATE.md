...
## 2026-09-12 — WP-20260912-A1 lezárva

A tegnapról bizonyított három háttér-helper eltérés byte-pontos visszakeresés után célzott `content/edit` patch-csel javítva, szerkezeti vagy ID-módosítás nélkül:

- page270: `af-s-rolam-9` **paper → sage**;
- page270: `af-s-rolam-11` **sage → paper**;
- page272: `af-s-soveny-specialista-8` **stone → sage**.

Mindhárom írás `replaced: 1` eredménnyel zárult. Ezeket a következő összevont visszaolvasásban újra ellenőrizni kell; külön cache-flush nem indult, mert a támogatott tartalmi patch kezelte a builder-oldali frissítést.

## Nyitott eltérések

A webhely **nem PIXEL_PASS**. Elsődleges bizonyíték: `reports/site-finalization-20260911.md`.

1. Galéria page258: a forrás `<main>` 86 képet tartalmaz (3 review + 39 kiemelt + 44 további); tegnapi target accessibility DOM 37 képet mutatott. A teljes forrás-médiakészletet és sorrendet vissza kell építeni a már meglévő médiatárból.
2. Sövény page272: a forrás `<main>` 52 képet tartalmaz (21 hero proof + 3 review + 28 referencia); tegnapi target accessibility DOM 16 képet mutatott. A teljes hero proof és referencia-galéria paritást helyre kell állítani.
3. `rolam` és `idos-fak-megmentese-szolgaltatas`: a forrásképek teljes paritását újra kell számolni, és a hiányzó képeket pótolni.
4. `referencia-videok` page268: mind a 7 videó jelen van, de a kiemelt videó Elementorban külön testvérként maradt a mért szekció mellett; a forrás vertikális struktúrájával össze kell vonni vagy célzottan kompenzálni.
5. Target screenshot/overlay/diff bizonyíték hiányzik. Az Opera Browser Connector DOM-olvasása működött, screenshot végpontja tegnap hibázott; ezt a médiarekonstrukciók után újra kell próbálni. PIXEL_PASS csak friss target képpárral, geometriával/overflow-val és kritikus editor reopen ellenőrzéssel adható.

## Aktív következő munkacsomag

**WP-20260912-B — page258 galéria forrásparitás.** A releváns AGENTS/D03/D07 szabályok, `specs/pages/galeria.json`, valamint a `before-after`, `manual-carousel`, `lightbox`, `masonry` behavior szerződések visszaolvasva. A friss Elementor 4.2.4 widgetleltár igazolja, hogy a natív Free `image`, `image-carousel` és `image-gallery` elemek rendelkezésre állnak; új addon telepítése ezért nem indokolt.

Munkamenet: aktuális page258 fa + élő média-ID/URL térkép egyszeri visszaolvasása → forrásképsorrend és viselkedés helyi rekonstrukciója → stabil meglévő ID-k megőrzésével egyetlen támogatott Elementor save → összevont adat/DOM/viselkedés-visszaellenőrzés. Ha a natív Free galéria/körhinta nem tud egy forrásviselkedést bizonyítottan, csak azt a hiányzó funkciót egészítheti ki a már meglévő `af-project` JS, nem kerülhet be új konkurens addon.

## További sorrend

1. page258 galéria teljes média- és viselkedésparitás;
2. page272 sövény hero proof + galéria;
3. page270 / page264 médiaaudit és szükséges pótlás;
4. page268 featured videó szerkezeti/geometriai javítása;
5. source↔target desktop/mobile vizuális QA az acceptance viewportokon, interakciók a `specs/behaviors.json` szerint;
6. kritikus Elementor editor reopen/editability/unsaved-change ellenőrzés;
...
A mostani friss 300-as induló keret mellett a terv **oldalanként egy összevont read → helyi validálás → egy save → egy visszaolvasás** ciklus, külön böngészős QA-val. A 10 hívásos vésztartalék végig megmarad. Ha a szolgáltatói számláló ettől eltérő fogyást mutat, a tényleges számláló az elsődleges.
